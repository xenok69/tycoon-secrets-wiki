...
export function SolutionBlock({ className, children, ...props }: ComponentProps<"div">) {
  ...
  const [flameRef, flameReady] = useVisibleOnce<HTMLDivElement>()
  ...
  const box = (
    <div
      ...
      className={cn(
        "relative flex min-h-20 w-full items-center justify-center rounded-sm bg-sidebar",
  ...
  return (
    <>
      {flameReady ? (
        ...
        <div className="my-4 w-full">{box}</div>
      )}
